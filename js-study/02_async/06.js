...
const start = async () => {
    // delay는 비동기함수(promise 반환하고있음)
    // await은 promise객체의 상태가 fullfill이 될떄까지
    // 해당 함수의 코드진행을 대기시킴 -> then과 동일
    try {
...
const getUserInfoById = async(userId) => {
    ...
    return userInfo; // userInfo가 promise안에 resolve됨
};

// getUserinfobyid(4) 작업이 끝나고, await을 써서
// resolve로 set했던 데이터를 받아 볼 수 있다.
